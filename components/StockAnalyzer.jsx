...
export default function StockAnalyzer() {
  ...
  const r=result,s=r?.scores||{},info=r?.stockInfo||{},bd=s.breakdown||{},ibd=s.industryBreakdown||{};
  const TABS=[{id:"indicators",label:"지표 상세"},{id:"industry",label:"업계 동향"},{id:"precautions",label:"유의사항"}];
  const isKRW=info.currency==="KRW";
  const fmt=v=>{if(!v||isNaN(v))return"-";const n=parseFloat(v);return isKRW?`₩${Math.round(n).toLocaleString()}`:`$${n.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`;};
  const fmtCap=v=>{if(!v)return"-";if(v>=1e12)return`$${(v/1e12).toFixed(2)}T`;if(v>=1e9)return`$${(v/1e9).toFixed(1)}B`;if(v>=1e8)return`${(v/1e8).toFixed(0)}억원`;return v.toLocaleString();};
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Sora',sans-serif",padding:"22px 16px",transition:"background .3s"}}>
      ...
      <div style={{maxWidth:880,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",gap:12,marginBottom:12,flexWrap:"wrap"}}>
              <div style={{...card(),flex:"1 1 280px"}}>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
                  ...
                  <span style={{background:T.neuBg,color:T.neu,border:`1px solid ${T.neuBdr}`,padding:"2px 9px",borderRadius:5,fontSize:10}}>{info.exchange||info.market}</span>
                ...
                <div style={{fontSize:28,fontWeight:800,fontFamily:"'IBM Plex Mono',monospace",color:T.accent,marginBottom:3}}>{fmt(info.currentPrice)}</div>
                <div style={{fontSize:11,color:T.textSub,marginBottom:10}}>시가총액 · {fmtCap(info.marketCap)}</div>
                <div style={{display:"flex",gap:16}}>
                  <div style={{fontSize:10,color:T.textFaint}}>52주 고 <span style={{color:T.neg,fontWeight:600}}>{fmt(info.yearHigh)}</span></div>
                  <div style={{fontSize:10,color:T.textFaint}}>52주 저 <span style={{color:T.pos,fontWeight:600}}>{fmt(info.yearLow)}</span></div>
                </div>
              </div>
              <div style={{...card(),flex:"1 1 230px",display:"flex",flexDirection:"column"}}>
        ...
        {!loading&&!r&&!error&&<div style={{textAlign:"center",padding:"52px 0"}}><div style={{fontSize:44,marginBottom:12,opacity:.2,color:T.accent}}>◈</div><div style={{fontSize:13,color:T.textSub,marginBottom:5}}>종목명 또는 티커를 입력해 분석을 시작하세요</div><div style={{fontSize:11,color:T.textFaint}}>NVDA · AAPL · 삼성전자 · 005930 · TSLA · 현대차</div></div>}
      </div>
    </div>
  );
}
